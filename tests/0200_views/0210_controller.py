...
def test_view_get_disabled(db, client, django_assert_num_queries, settings):
    """
    When disabled a GET request receives either a specific page for disabled form
    or a Http404.
    """
    controller = ControllerFactory(enabled=False)
    ...
    settings.CANOPY_CONTROLLER_EXCEPTION_WHEN_DISABLED = True
...
def test_view_get_enabled(db, client, django_assert_num_queries):
    """
...
def test_view_success_disabled(db, client, django_assert_num_queries, settings):
    """
    When disabled the success view returns either a specific page for disabled form
    or a Http404.
    """
    controller = ControllerFactory(enabled=False)
    SlotFactory(controller=controller, name="basic-text", required=True)

    url = reverse("canopy:controller-success", kwargs={"slug": controller.slug})
...
def test_view_post_disabled(db, client, django_assert_num_queries, settings):
    """
    When disabled a POST request receives either a specific page for disabled form
    or a Http404.
    """
    controller = ControllerFactory(enabled=False)
    SlotFactory(controller=controller, name="basic-text", required=True)
...
def test_view_post_enabled(db, client, django_assert_num_queries):
    """
